...
import re
import yaml
from formatter import format_api_response
...
def phase1_detect_intent(user_input, api_catalog, history, valid_api_ids, apis=None):
    # Layer 0 — Regex NLP parser (extremely fast, zero LLM load)
    try:
        ...
        if intent_res.intent != "unknown":
            mapped = map_nlp_to_api(intent_res, valid_api_ids)
            if mapped:
                return {
                    "api_id": mapped["api_id"],
                    "confidence": "high",
                    "reason": f"nlp regex match (intent={intent_res.intent})",
                    "reply": None,
                    "extracted_entities": mapped["extracted_entities"]
                }
    except Exception as e:
        import os
        if os.getenv("LLM_DEBUG"):
            print(f"[debug] NLP parser failed: {e}", flush=True)
    ...
    try:
        ...
        if mapped and mapped["extracted_entities"]:
            for key, val in mapped["extracted_entities"].items():
                if allowed_names is None or key in allowed_names:
                    regex_extracted[key] = val
    except Exception:
        pass

    # Skip invoking LLM if we have already extracted all allowed required/optional parameters
    if allowed_names and set(allowed_names).issubset(set(regex_extracted.keys())):
        parsed = regex_extracted
    else:
        llm_out = safe_json_parse(llm_extract.invoke(prompt).content) or {}
        ...
        for key, val in regex_extracted.items():
            parsed[key] = val
...
def get_missing_params(api, collected):
    try:
        ...
        global_params = registry.get("global_parameters", {})
...
def phase4_explain(api, raw_response, original_query):
    ...
    formatted = format_api_response(api, raw_response)
...
def execute_and_explain(api, converted_params, tool_map, original_query):
    ...
    formatted = format_api_response(api, result)
    if formatted is not None:
        return formatted
