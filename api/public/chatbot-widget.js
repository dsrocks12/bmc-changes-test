...
class BmcChatbotWidget extends HTMLElement {
  ...
  formatMessageText(text) {
    ...
    // Parse Markdown tables and code blocks
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.startsWith('```')) {
        if (inCodeBlock) {
          // Close the code block and wrap in terminal console layout
          processedLines.push(`
            <div class="diagnostic-log-container">
              <div class="diagnostic-log-header">
                <span class="diagnostic-log-title">Console Output</span>
              </div>
              <pre class="chat-code-block"><code>${codeBlockLines.join('\n')}</code></pre>
            </div>
          `);
          codeBlockLines = [];
          inCodeBlock = false;
      ...
      if (inCodeBlock) {
        // Retain original line spaces inside code block
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .chat-code-block {
          ...
          font-size: 0.82rem;
          color: #38bdf8; /* Sleek terminal blue */
          line-height: 1.5;
          white-space: pre;
        }
